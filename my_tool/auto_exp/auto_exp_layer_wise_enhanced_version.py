...
def process_run(args):
    ...
    for index, ratios_group in enumerate(ratios_groups):      # ratios_group (shape): {blocks_num, lora num}
        locked_keys = TE_LOCKEDKEYS + UNE_LOCKEDKEYS if args.locked_keys is None else args.locked_keys
        ...
        cmd = rf"D:\tool\Anaconda3\envs\LoRA\python.exe  D:\seekoo\SD\sd-scripts\my_tool\merge_param.py --models {models} --save_dir=E:\Data\test\Multi-LoRA-{len(model_names)}\Seed_{args.seed}_Index_{sample_index} --remark_info {sample_index} --locked_keys {locked_keys} --multi_lora_ratios {ratios} --seed {args.seed}"
