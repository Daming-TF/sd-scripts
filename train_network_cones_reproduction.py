...
def train(args):
    ...
    # mixed precision 准备好对应的模板，适当地进行cast
    weight_dtype, save_dtype = train_util.prepare_dtype(args)

    # 读取模型
    text_encoder, vae, unet, _ = train_util.load_target_model(args, weight_dtype, accelerator)

    # 在模型中加入xformers或memory efficient attention
    ...
    if args.base_weights is not None:
        # base_weights 如果指定了，则读取并合并指定的权重
    ...
    if args.dim_from_weights:
        network, _ = network_module.create_network_from_weights(1, args.network_weights, vae, text_encoder, unet,
                                                                for_inference=args.inference_debug, **net_kwargs)
    else:
...
def setup_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        default=None,
        nargs="*",
        help="network weights to merge into the model before training / 学习前预先合并到模型中的network权重文件",
    )
    parser.add_argument(
        ...
        default=None,
        nargs="*",
        help="multiplier for network weights to merge into the model before training / 学习前预先合并到模型中的network权重的倍率",
    )
    ...
    parser.add_argument(
        "--inference_debug",
        action="store_true",
        help="Debug Enable——only load LoRA for inference",
    )
    return parser
